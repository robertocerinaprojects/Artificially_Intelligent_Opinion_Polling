...
reg <-
  as.data.table(
    read_xlsx(
      path = 'data_auxiliary/registration_and_turnout_2020.xlsx',
      skip = 4
      )
    )
...
obs_dt <-
  reshape(
    obs_dt[,c('state','pct_vote','party_detailed'),with = F],
...
# save results data
save(obs_dt, file = 'data_generated/observed_vote.RData',compress = TRUE)
