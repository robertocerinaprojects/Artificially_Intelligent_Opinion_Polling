...
obs_dt$other <- 1-(obs_dt$D+obs_dt$R)
obs_dt <- obs_dt[,!c('L','G')]
obs_dt$R.D_margin <- obs_dt$R - obs_dt$D
obs_dt$dte = 0
obs_dt <- melt(obs_dt[,!c('registered','turnout')],id.vars = c('state_simple','dte'),variable.name = 'choice',value.name = 'observed')

load(file = 'data_generated/area_preds_f38_comparison.RData')
area.pred_list <- area.pred_list_f38
...
levels(area.pred_list$training_data)[
  ...
  levels(area.pred_list$training_data)[
  which(levels(area.pred_list$training_data)=="538_forecast")
  ] = '9.FiveThirtyEight'
...
levels(pred_sims_delta$training_data)[
  which(levels(pred_sims_delta$training_data)=="538_forecast")
  ] = '9.FiveThirtyEight'
...
pdf(file = 'plots/pred_v_obs_538.pdf',width = 15,height = 7.5)
plot_preds_v_obs(area.pred_list)
dev.off()

# plot predictions against observations
pdf(file = 'plots/delta_pred_v_obs_538.pdf',width = 12.5,height = 7.5)
...
fwrite(
  performance,
  file = 'data_generated/performance_538.csv'
  )